...
{	name: "user_profile",
waitOn: function() {
	this.subscribe('userProfile', this.params._id);
...
Router.route("/my-profile/:_id", function(){
...
{	name: "my-profile",
	...
	data: function(){
		...
		var recievedMessages = MessagesPagination.find({sent_to: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var pagRecMessages = MessagesPagination.find({sent_to: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var replies_to_user_messages = Replies.find({sent_to: _id}, {sort: {createdAt: -1}}).fetch();
		var unread_replies_for_user_messages = [];
		var unreadMessages = [];
		var  items = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var paginationItems = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var oddItems = [];
		var evenItems = [];
		var trips = TripsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var paginationTrips = TripsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var evenTrips = [];
		var oddTrips = [];
		var rec_req_carry_user_item = RequestsPagination.find({carrierId: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var rec_req_carry_your_item = RequestsPagination.find({senderId: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var sent_requests = RequestsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		// var requests = rec_req_carry_user_item.concat(rec_req_carry_your_item, sent_requests);
		var requests = RequestsPagination.find( { $or: [ { carrierId: _id }, { senderId: _id }, {owner: _id} ] }, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var pagRequests = RequestsPagination.find( { $or: [ { carrierId: _id }, { senderId: _id }, {owner: _id} ] }, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var unreadRequests = [];
		var openRequests = [];
		var closedRequests = [];
		var notifications = NotificationsPagination.find({recipient: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var pagNotifications = NotificationsPagination.find({recipient: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		...
		$(requests).each(function(index, value) {
			if (value.read_status == "unread" && value.type != "bulk_req_carry") {
				unreadRequests.push(value);
			};
		});

		$(requests).each(function(index, value) {
			if (value.action_status == "open" && value.type != "bulk_req_carry") {
				openRequests.push(value);
			};
		});

		$(requests).each(function(index, value) {
			if (value.action_status == "closed" && value.type != "bulk_req_carry") {
...
Router.route("/my-profile/:_id/notifications", function(){
...
{	name: "notifications",
	...
	data: function(){
		...
		var unread_replies_for_user_messages = [];
		var unreadMessages = [];
		var  items = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var paginationItems = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var oddItems = [];
		var evenItems = [];
		var trips = TripsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
...
Router.route("/my-profile/:userId/notification/:_id", function(){
	this.render("notificationView");
},
{	name: "notification-view",
	waitOn: function(){
		this.subscribe('travels');
		this.subscribe('userProfile', this.params.userId);
...
Router.route("/my-profile/:_id/requests-home", function(){
...
{	name: "request-details",
	...
	data: function(){
		...
		var unreadMessages = [];
		var  items = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var paginationItems = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var oddItems = [];
		var evenItems = [];
		var trips = TripsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var paginationTrips = TripsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
...
Router.route("/my-profile/:_id/messages-home", function(){
...
Router.route("/my-profile/:_id/sent-messages", function(){
...
{	name: "sent-messages",
	...
	data: function(){
		...
		var recievedMessages = MessagesPagination.find({sent_to: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var pagRecMessages = MessagesPagination.find({sent_to: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var userSentMessages = MessagesPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
		var pagUserSentMessages = MessagesPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
		var unreadMessages = [];
		var replies_to_user_messages = Replies.find({sent_to: _id}).fetch();
		var unread_replies_for_user_messages = [];
		var  items = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
...
data: function(){
	...
	var replies = Replies.find({messageId: this.params._id}, {sort: {createdAt: -1}}).fetch();
	var unreadReplies = [];
	var replies_to_user_messages = Replies.find({sent_to: _id}, {sort: {createdAt: -1}}).fetch();
	var unread_replies_for_user_messages = [];
	var recievedMessages = MessagesPagination.find({sent_to: _id}, {sort: {createdAt: -1}}, {itemsPerPage:1});
	var pagRecMessages = MessagesPagination.find({sent_to: _id}, {sort: {createdAt: -1}}, {itemsPerPage:1}).fetch();
	var unreadMessages = [];
	var  items = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
	var paginationItems = ItemsPagination.find({owner: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
	var oddItems = [];
	...
	var openRequests = [];
	var closedRequests = [];
	var messages = Messages.find({sent_to: _id}, {sort: {createdAt: -1}});
	var notifications = NotificationsPagination.find({recipient: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10}).fetch();
	var pagNotifications = NotificationsPagination.find({recipient: _id}, {sort: {createdAt: -1}}, {itemsPerPage:10});
	var unreadNotifications = [];
...
Router.route(":_id/edit/profile", function(){
...
data: function() {
	var trips = Travels.find({}, {sort: {createdAt: -1}}).fetch();
	var evenTrips = [];
	var oddTrips = [];
...
data: function() {
	...
	var items = Items.find({origin_country: trip_origin_country, destination_country: trip_destination_country}, {sort: {createdAt: -1}}).fetch();
